...
function getRandomPhoneNumber() {
  ...
  const exchangeCode =
    Math.floor(Math.random() * 900) + 100; // Random number between 100 and 999
...
async function setCookie(page, name, value) {
  await page
    .context()
    .addCookies([{ name, value, url: page.url() }]);
}
...
const getSelector = (string, del = '.') =>
  string
    .split(del)
    .reduce((acc, key) => acc[key], locators);

const getData = (string, del = '.') =>
  string.split(del).reduce((acc, key) => acc[key], data);
