...
When('I right-click on the {string}', async function (selector) {
  await this.page.click(locators[selector.split(".")[0]][selector.split(".")[1]], { button: 'right' });
});
...
Then('The {string} radio button should be selected', async function (selector) {
  const isChecked = await this.page.isChecked(locators[selector.split(".")[0]][selector.split(".")[1]]);
...
When('I scroll to the {string}', async function (selector) {
  await this.page.locator(locators[selector.split(".")[0]][selector.split(".")[1]]).scrollIntoViewIfNeeded();
});
...
Then("I should see the {string} element", async function (selector) {
  expect(await this.page.isVisible(locators[selector.split(".")[0]][selector.split(".")[1]])).toBeTruthy();
});

// Example: Verifying an element's visibility
Then('the {string} should not be visible', async function (selector) {
  const isVisible = await this.page.isVisible(locators[selector.split(".")[0]][selector.split(".")[1]]);
  expect(isVisible).toBeFalsy();
});

// Example: Check if an element is enabled
Then('the {string} should be enabled', async function (selector) {
  const isEnabled = await this.page.isEnabled(locators[selector.split(".")[0]][selector.split(".")[1]]);
...
Then('the {string} field should be empty', async function (selector) {
  const value = await this.page.inputValue(locators[selector.split(".")[0]][selector.split(".")[1]]);
  expect(value).toBe('');
});

// Example: Check if an element's attribute has a specific value
Then('the {string} attribute of the {string} should be {string}', async function (attribute, selector, expectedValue) {
  const actualValue = await this.page.getAttribute(locators[selector.split(".")[0]][selector.split(".")[1]], attribute);
...
Then('The element {string} should contain the text {string}', async function (selector, text) {
  const elementText = await this.page.textContent(locators[selector.split(".")[0]][selector.split(".")[1]]);
...
When('I wait for the {string} attribute of the {string} to be {string}', async function (attribute, selector, value) {
  await this.page.waitForFunction(
...
When('I wait for the {string} to be visible', async function (selector) {
  await this.page.waitForSelector(locators[selector.split(".")[0]][selector.split(".")[1]], { state: 'visible' });
});

// Example: Wait for a specific element to disappear
When('I wait for the {string} to disappear', async function (selector) {
  await this.page.waitForSelector(locators[selector.split(".")[0]][selector.split(".")[1]], { state: 'hidden' });
});
